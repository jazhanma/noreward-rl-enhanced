    ...
    try:
        ...
        if hasattr(env, 'reset') and hasattr(env, 'action_space'):
            ...
            if hasattr(env, 'unwrapped'):
                ...
                print("⚠️  Warning: Could not seed environment {type(env).__name__}")
                actual_seed = None

    except Exception as e:
        print("⚠️  Warning: Failed to seed environment: {e}")
        actual_seed = None
...
def test_reproducibility(seed: int = 42, num_steps: int = 10) -> None:
    ...
    print("🧪 Reproducibility test (seed={seed}):")
    print("   Python random: {'✅' if python_reproducible else '❌'}")
    print("   NumPy random: {'✅' if numpy_reproducible else '❌'}")
    print("   TensorFlow random: {'✅' if tf_reproducible else '❌'}")
    print("   Overall: {'✅' if all_reproducible else '❌'}")
