...
class ConfigManager:
    ...
    def __init__(self, config_dir: Union[str, Path] = "configs"):
        ...
        logger.info("Config manager initialized: {self.config_dir}")
    ...
    def load_environment_config(self, env_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info("Loaded environment config: {env_name}")
            return config

        except yaml.YAMLError as e:
            raise ConfigError("Invalid YAML in {config_file}: {e}")
        except Exception as e:
            raise ConfigError("Error loading config {config_file}: {e}")
    ...
    def load_experiment_config(self, exp_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info("Loaded experiment config: {exp_name}")
            return config
    ...
    def load_combined_config(
        ...
        try:
            ...
            logger.warning("Environment config not found: {env_name}, using defaults")
            env_config = {}
        ...
        logger.info("Loaded combined config: {exp_name} + {env_name}")
    ...
    def _validate_environment_config(self, config: Dict[str, Any]) -> None:
        ...
        for field in required_fields:
            if field not in config:
                raise ConfigError("Missing required field in environment config: {field}")
    ...
    def _validate_experiment_config(self, config: Dict[str, Any]) -> None:
        ...
        for field in required_fields:
            if field not in config:
                raise ConfigError("Missing required field in experiment config: {field}")
    ...
    def _validate_merged_config(self, config: Dict[str, Any]) -> None:
        ...
        for section in required_sections:
            if section not in config:
                raise ConfigError("Missing required section in merged config: {section}")
    ...
    def save_config(self, config: Dict[str, Any], filepath: Union[str, Path]) -> None:
        ...
        try:
            ...
            raise ConfigError("Error saving config to {filepath}: {e}")
...
if __name__ == "__main__":
    ...
    print("Environment configs: {env_configs}")
    print("Experiment configs: {exp_configs}")

    # Test loading configs
    try:
        env_config = manager.load_environment_config("cartpole")
        print("✅ Loaded environment config: {env_config['name']}")

        exp_config = manager.load_experiment_config("curiosity_baseline")
        print("✅ Loaded experiment config: {exp_config['experiment']['name']}")

        combined_config = manager.load_combined_config("curiosity_baseline", "cartpole")
        print("✅ Loaded combined config successfully")
