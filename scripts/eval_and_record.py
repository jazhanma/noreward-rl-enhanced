...
class AgentEvaluator:
    ...
    def __init__(
        ...
        self.logger = create_logger(
            ...
            experiment_name=experiment_name or f"eval-{env_id}-{int(time.time())}",
            use_wandb=use_wandb,
    ...
    def _load_model(self) -> LSTMPolicy:
        ...
        print(f"Model loaded successfully. Global step: {sess.run(policy.global_step)}")
    ...
    def evaluate(
        ...
        print(f"Policy: {'greedy' if greedy else 'random' if random else 'sampled'}")
        ...
        if record:
            self.env = gym.wrappers.RecordVideo(
                ...
                name_prefix=f"{self.env_id}_episode"
            )
    ...
    def _run_episode(
        ...
        if save_frames:
            frames_dir = self.output_dir / "frames" / f"episode_{episode_num:03d}"
            frames_dir.mkdir(exist_ok=True)
            frame_count = 0
        ...
        while True:
            ...
            if save_frames:
                ...
                if frame is not None:
                    Image.fromarray(frame).save(frames_dir / f"frame_{frame_count:06d}.png")
                    frame_count += 1
    ...
    def _save_results(self, results: Dict[str, Any]) -> None:
        ...
        with open(self.output_dir / "results.txt", "w") as f:
            f.write(f"Evaluation Results for {self.env_id}\n")
            f.write("=" * 50 + "\n")
    ...
    def _print_summary(self, results: Dict[str, Any]) -> None:
        ...
        """
        print("\n" + "=" * 50)
        print(f"EVALUATION SUMMARY - {self.env_id.upper()}")
        print("=" * 50)
        print(f"Episodes: {results['num_episodes']}")
        print(f"Mean Reward: {results['mean_reward']:.2f} ± {results['std_reward']:.2f}")
        print(f"Min/Max Reward: {results['min_reward']:.2f} / {results['max_reward']:.2f}")
        print(f"Mean Length: {results['mean_length']:.1f} ± {results['std_length']:.1f}")
        print(f"Mean Time: {results['mean_time']:.2f}s ± {results['std_time']:.2f}s")
        print(f"Rewards/Second: {results['rewards_per_second']:.2f}")
        print("=" * 50)
