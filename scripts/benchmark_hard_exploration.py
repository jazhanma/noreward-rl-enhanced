...
class HardExplorationBenchmark:
    ...
    def benchmark_game(
        ...
        """
        print("\n{'='*60}")
        print("BENCHMARKING: {env_id.upper()}")
        print("{'='*60}")
        ...
        evaluator = AgentEvaluator(
            ...
            experiment_name="benchmark-{env_id}",
        )
    ...
    def benchmark_all_games(
        ...
        """
        print("BENCHMARKING ALL HARD EXPLORATION GAMES")
        print("Games: {len(HARD_EXPLORATION_GAMES)}")
        print("Episodes per game: {num_episodes}")
        print("Total episodes: {len(HARD_EXPLORATION_GAMES) * num_episodes}")
        ...
        for i, game in enumerate(HARD_EXPLORATION_GAMES):
            ...
            try:
                ...
                print("Error benchmarking {game}: {e}")
    ...
    def _save_report(self, summary: Dict[str, Any]) -> None:
        ...
        with open(report_path, "w") as f:
            ...
            f.write("Model: {self.model_path}\n")
            f.write("Timestamp: {time.strftime('%Y-%m-%d %H:%M:%S')}\n\n")

            f.write("OVERALL STATISTICS\n")
            f.write("-" * 20 + "\n")
            f.write("Total Games: {summary['total_games']}\n")
            f.write("Successful: {summary['successful_games']}\n")
            f.write("Failed: {summary['failed_games']}\n")

            if summary['successful_games'] > 0:
                overall = summary['overall_stats']
                f.write("Mean Reward: {overall['mean_reward']:.2f} ± {overall['std_reward']:.2f}\n")
                f.write("Mean Length: {overall['mean_length']:.1f}\n")
                f.write("Total Episodes: {overall['total_episodes']}\n")
            ...
            for game, stats in summary['game_stats'].items():
                f.write("\n{game}:\n")
                f.write("  Episodes: {stats['num_episodes']}\n")
                f.write("  Mean Reward: {stats['mean_reward']:.2f} ± {stats['std_reward']:.2f}\n")
                f.write("  Mean Length: {stats['mean_length']:.1f}\n")

            if summary['errors']:
                f.write("\nERRORS\n")
                f.write("-" * 10 + "\n")
                for game, error in summary['errors'].items():
                    f.write("{game}: {error['error']}\n")

        print("Benchmark report saved to: {report_path}")
