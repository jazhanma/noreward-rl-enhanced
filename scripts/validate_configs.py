...
def validate_all_configs(config_dir: str = "configs") -> Tuple[bool, List[str]]:
    ...
    print("Found {len(env_configs)} environment configs: {env_configs}")
    print("Found {len(exp_configs)} experiment configs: {exp_configs}")
    print()
    ...
    for env_name in env_configs:
        try:
            ...
            print("  ✅ {env_name}: {config.get('name', 'Unknown')}")

            # Additional validation
            if 'training' in config:
                training = config['training']
                if 'learning_rate' in training and training['learning_rate'] <= 0:
                    errors.append("Environment {env_name}: Invalid learning_rate {training['learning_rate']}")
                if 'max_episodes' in training and training['max_episodes'] <= 0:
                    errors.append("Environment {env_name}: Invalid max_episodes {training['max_episodes']}")

        except ConfigError as e:
            error_msg = "Environment {env_name}: {e}"
            errors.append(error_msg)
            print("  ❌ {env_name}: {e}")
        except Exception as e:
            error_msg = "Environment {env_name}: Unexpected error - {e}"
            errors.append(error_msg)
            print("  ❌ {env_name}: Unexpected error - {e}")
    ...
    for exp_name in exp_configs:
        try:
            ...
            print("  ✅ {exp_name}: {config.get('experiment', {}).get('name', 'Unknown')}")

            # Additional validation
            training = config.get('training', {})
            if 'learning_rate' in training and training['learning_rate'] <= 0:
                errors.append("Experiment {exp_name}: Invalid learning_rate {training['learning_rate']}")
            if 'max_global_steps' in training and training['max_global_steps'] <= 0:
                errors.append("Experiment {exp_name}: Invalid max_global_steps {training['max_global_steps']}")
            if 'num_workers' in training and training['num_workers'] <= 0:
                errors.append("Experiment {exp_name}: Invalid num_workers {training['num_workers']}")

        except ConfigError as e:
            error_msg = "Experiment {exp_name}: {e}"
            errors.append(error_msg)
            print("  ❌ {exp_name}: {e}")
        except Exception as e:
            error_msg = "Experiment {exp_name}: Unexpected error - {e}"
            errors.append(error_msg)
            print("  ❌ {exp_name}: Unexpected error - {e}")
    ...
    for exp_name in exp_configs:
        try:
            ...
            print("  ✅ {exp_name} + default env: {env_id}")

            # Test with specific environments
            for env_name in env_configs:
                try:
                    combined_config = manager.load_combined_config(exp_name, env_name)
                    print("    ✅ {exp_name} + {env_name}: OK")
                except ConfigError as e:
                    error_msg = "Combined {exp_name} + {env_name}: {e}"
                    errors.append(error_msg)
                    print("    ❌ {exp_name} + {env_name}: {e}")

        except ConfigError as e:
            error_msg = "Combined {exp_name}: {e}"
            errors.append(error_msg)
            print("  ❌ {exp_name}: {e}")
        except Exception as e:
            error_msg = "Combined {exp_name}: Unexpected error - {e}"
            errors.append(error_msg)
            print("  ❌ {exp_name}: Unexpected error - {e}")
...
def validate_specific_config(config_path: str) -> bool:
    ...
    try:
        ...
        print("❌ Error validating {config_path}: {e}")
        return False
