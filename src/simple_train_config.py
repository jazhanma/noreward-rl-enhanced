...
def simple_training_loop_with_config(
    ...
    print(f"🚀 Starting configuration-driven training on {env_id}")
    print("=" * 60)
    print(f"Experiment: {exp_config.get('name', 'unknown')}")
    print(f"Description: {exp_config.get('description', 'No description')}")
    print(f"Episodes: {num_episodes}")
    print(f"Use W&B: {use_wandb}")
    print(f"Seed: {seed if seed is not None else 'Random'}")
    print(f"Save Directory: {save_dir if save_dir else 'Disabled'}")
    print("=" * 60)
    ...
    print(f"Environment config: {env_config_dict}")
    print(f"Global constants: GAMMA={constants['GAMMA']}, LR={constants['LEARNING_RATE']}")
    print(f"Entropy Beta: {constants['ENTROPY_BETA']}, Prediction Beta: {constants['PREDICTION_BETA']}")
    
    # Create logger
    experiment_name = logging_config.get('experiment_name', f"config-{env_id}-{int(time.time())}")
    ...
    if save_dir is not None:
        checkpoint_manager = create_checkpoint_manager(
            ...
            checkpoint_prefix=f"config_{env_id}"
        )
    ...
    if seed is not None:
        ...
        if actual_seed is not None:
            print(f"🌱 Environment seeded with: {actual_seed}")
        else:
    ...
    for episode in range(start_episode, start_episode + num_episodes):
        ...
        # Log episode results
        if episode % log_interval == 0 or episode == start_episode:
            print(f"  Reward: {episode_reward:.2f}, Length: {episode_length}, Time: {episode_time:.2f}s")
    ...
    if targets:
        ...
        if solved_reward and max(episode_rewards) >= solved_reward:
            print(f"✅ Solved! (Target: {solved_reward}, Best: {max(episode_rewards):.2f})")
        elif solved_reward:
            print(f"❌ Not solved (Target: {solved_reward}, Best: {max(episode_rewards):.2f})")
...
def main():
    ...
    try:
        ...
        if args.env_id:
            ...
            print(f"🔄 Override: Environment ID = {args.env_id}")
        
        if args.num_episodes:
            config['training']['max_episodes'] = args.num_episodes
            print(f"🔄 Override: Episodes = {args.num_episodes}")
        ...
        if args.save_dir:
            ...
            print(f"🔄 Override: Save Directory = {args.save_dir}")
