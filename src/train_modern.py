...
def train_worker(
    ...
    if config.get('seed') is not None:
        ...
        if actual_seed is not None:
            print(f"🌱 Worker {worker_id} environment seeded with: {actual_seed}")
        else:
            print(f"⚠️  Warning: Could not seed environment for worker {worker_id}")
    ...
    if config.get('save_dir') is not None:
        checkpoint_manager = create_checkpoint_manager(
            ...
            checkpoint_prefix=f"worker_{worker_id}_{config['env_id']}"
        )
        ...
        if config.get('resume', False):
            ...
            if latest_checkpoint:
                print(f"🔄 Worker {worker_id} resuming from checkpoint: {latest_checkpoint.name}")
            else:
                print(f"⚠️  Worker {worker_id}: No checkpoint found, starting from beginning")
    ...
    if config['use_tensorboard']:
        log_dir = Path(config['log_dir']) / f"worker_{worker_id}"
    ...
    with tf.Session() as sess:
        ...
        print(f"Worker {worker_id} starting at global step: {global_step}")
        ...
        while global_step < constants['MAX_GLOBAL_STEPS']:
            ...
            if global_step % 1000 == 0:
                print(f"Worker {worker_id} - Global step: {global_step}")
...
def main():
    ...
    if config['use_wandb'] or config['use_tensorboard']:
        logger = create_logger(
            ...
            experiment_name=config['experiment_name'] or f"train-{config['env_id']}-{int(time.time())}",
    ...
    if config['num_workers'] == 1:
        ...
        print(f"Multi-worker training not implemented in this simplified version")
        print(f"Using single worker instead")
        train_worker(0, config, logger)
