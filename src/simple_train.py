...
def simple_training_loop(
    ...
    """
    print(f"🚀 Starting simple training on {env_id}")
    print("=" * 50)
    ...
    logger = create_logger(
        ...
        experiment_name=f"simple-{env_id}-{int(time.time())}",
        use_wandb=use_wandb,
    ...
    if save_dir is not None:
        checkpoint_manager = create_checkpoint_manager(
            ...
            checkpoint_prefix=f"simple_{env_id}"
        )
...
def main():
    ...
    print(f"Save Directory: {args.save_dir if args.save_dir else 'Disabled'}")
